...
Rasterization::Rasterization(std::shared_ptr<DIBSection> pDIBSection) : m_pDIBSection(pDIBSection)
{
...
void Rasterization::DrawHLine(int x0, int x1, int y, const RGBColor& c)
{
	...
	for (int x = x0; x <= x1; ++x)
	{
		m_pDIBSection->SetPixel(x, y, GetRGBRed(c), GetRGBGreen(c), GetRGBBlue(c));
	}
}
...
void Rasterization::DrawHLine(int x0, float z0, int x1, float z1, int y, const RGBColor& c)
{
	...
	for (int x = x0, t = 0; x <= x1; ++x, ++t)
	{
		...
		m_pDIBSection->SetZBuffer(x, y, z, GetRGBRed(c), GetRGBGreen(c), GetRGBBlue(c));
	}
}
...
void Rasterization::DrawHLine(int x0, int x1, int y, const RGBColor& c0, const RGBColor& c1)
{
	...
	for (int x = x0, t = 0; x <= x1; ++x, ++t)
	{
		...
		m_pDIBSection->SetPixel(x, y, GetRGBRed(color), GetRGBGreen(color), GetRGBBlue(color));
	}
}
...
void Rasterization::DrawHLine(int x0, float z0, int x1, float z1, int y, const RGBColor& c0, const RGBColor& c1)
{
	...
	for (int x = x0, t = 0; x <= x1; ++x, ++t)
	{
		color = Interpolation(color0, color1, t, length);
		z = Interpolation(z0, z1, t, length);
		m_pDIBSection->SetZBuffer(x, y, z, GetRGBRed(color), GetRGBGreen(color), GetRGBBlue(color));
	}
}
...
void Rasterization::DrawLine(const Vector2i& v0, const Vector2i& v1, const RGBColor& c)
{
	...
	if (isYLonger)
	{
		...
		{
			m_pDIBSection->SetPixel(x >> 16, y, GetRGBRed(c), GetRGBGreen(c), GetRGBBlue(c));
			x += bias * increase;
			y += bias;
		}
	}
	else
	{
		...
		{
			m_pDIBSection->SetPixel(x, y >> 16, GetRGBRed(c), GetRGBGreen(c), GetRGBBlue(c));
			y += bias * increase;
			x += bias;
		}
	}
...
void Rasterization::DrawLine(const Vector2i& v0, float z0, const Vector2i& v1, float z1, const RGBColor& c)
{
	...
	if (isYLonger)
	{
		...
		while ((y0 <= y && y <= y1) || (y1 <= y && y <= y0))
		{
			...
			m_pDIBSection->SetZBuffer(x >> 16, y, z, GetRGBRed(c), GetRGBGreen(c), GetRGBBlue(c));
			x += bias * increase;
			y += bias;
			t += 1;
		}
	...
	else
	{
		...
		while ((x0 <= x && x <= x1) || (x1 <= x && x <= x0))
		{
			...
			m_pDIBSection->SetZBuffer(x, y >> 16, z, GetRGBRed(c), GetRGBGreen(c), GetRGBBlue(c));
			y += bias * increase;
			x += bias;
			t += 1;
		}
...
void Rasterization::DrawLine(const Vector2i& v0, const RGBColor& c0, const Vector2i& v1, const RGBColor& c1)
{
	...
	if (isYLonger)
	{
		...
		{
			color = Interpolation(color0, color1, t, length);
			m_pDIBSection->SetPixel(x >> 16, y, GetRGBRed(color), GetRGBGreen(color), GetRGBBlue(color));
	...
	else
	{
		...
		{
			color = Interpolation(color0, color1, t, length);
			m_pDIBSection->SetPixel(x, y >> 16, GetRGBRed(color), GetRGBGreen(color), GetRGBBlue(color));
...
void Rasterization::DrawLine(const Vector2i& v0, float z0, const RGBColor& c0, const Vector2i& v1, float z1, const RGBColor& c1)
{
	...
	if (isYLonger)
	{
		...
		while ((y0 <= y && y <= y1) || (y1 <= y && y <= y0))
		{
			...
			m_pDIBSection->SetZBuffer(x >> 16, y, z, GetRGBRed(color), GetRGBGreen(color), GetRGBBlue(color));
	...
	else
	{
		...
		while ((x0 <= x && x <= x1) || (x1 <= x && x <= x0))
		{
			...
			m_pDIBSection->SetZBuffer(x, y >> 16, z, GetRGBRed(color), GetRGBGreen(color), GetRGBBlue(color));
...
void Rasterization::DrawCircle(const Vector2i& center, int radius, const RGBColor& c)
{
	int circleX = 0;
	int circleY = radius;
	int increaseDecision = 1 - radius;
	int deltaE = 3;
	int deltaSE = -2 * radius + 5;

	while (circleY > circleX)
	{
		if (increaseDecision < 0)
		{
			increaseDecision += deltaE;
			deltaE += 2;
			deltaSE += 2;
		}
		else
		{
			increaseDecision += deltaSE;
			deltaE += 2;
			deltaSE += 4;
			--circleY;
		}

		m_pDIBSection->SetPixel(center.x + circleX, center.y + circleY, GetRGBRed(c), GetRGBGreen(c), GetRGBBlue(c));
		m_pDIBSection->SetPixel(center.x + circleY, center.y + circleX, GetRGBRed(c), GetRGBGreen(c), GetRGBBlue(c));
...
void Rasterization::FillCircle(const Vector2i& center, int radius, const RGBColor& c)
{
	int circleX = 0;
	int circleY = radius;
	int increaseDecision = 1 - radius;
	...
	while (circleY > circleX)
	{
		...
		DrawHLine(center.x - circleX, center.x + circleX, center.y + circleY, c);
		DrawHLine(center.x - circleY, center.x + circleY, center.y + circleX, c);
		DrawHLine(center.x - circleX, center.x + circleX, center.y - circleY, c);
		DrawHLine(center.x - circleY, center.x + circleY, center.y - circleX, c);
